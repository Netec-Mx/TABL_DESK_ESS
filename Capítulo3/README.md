...
Mediante el uso de filtros y colores, puedes centrarte más en los detalles que te interesan. Después de centrarte en ciertos datos, puedes empezar a utilizar otras características de Tableau Desktop para interactuar con esos datos.
...
#### Revisa tu avance: observa `Aplicar filtros a la vista` en acción
...
#### Revisa tu avance: observa "Agregar color a la vista" en acción
...
Además, los campos `Latitud` y `Longitud` se agregan a los estantes `Columnas` y `Filas`. Puedes considerarlos como campos _X_ y e _Y_. Son esenciales para cuando quiera crear una vista de mapa, porque cada ubicación de los datos se asigna a un valor de latitud y de longitud. A veces, los campos `Latitud` y `Longitud` los genera Tableau. Otras veces, debes incluirlos manualmente en los datos. En la _Biblioteca de aprendizaje_ encontrarás recursos para profundizar más en este tema.



3. Para profundizar lo que deseas saber qué ocurría en la región sur, arrastra `Region` al estante `Filtros`  y, después, filtra solo por `South`. La vista de mapa amplía la región `South`, donde hay una marca para cada estado (11 en total).

Ahora puedes ver datos más detallados para esta región, así que empieza a arrastrar otros campos a la tarjeta `Marcas`.

4. Arrastra la medida `Sales` a `Color`en la tarjeta `Marcas`.

<img src="../images/03/15.gif" width="800" >

La vista se actualiza automáticamente a un mapa rellenado, donde cada estado está representado por un color según el total de ventas. Debido a que estás explorando las ventas de productos, sus ventas deben aparecer en USD. Haz clic en el campo `Sum(Sales)` en el estante `Columnas` y selecciona `Formato`. En `Números`, selecciona `Moneda`.

Cada vez que añades una medida continua que contiene número positivos (por ejemplo, `Ventas` ) a `Color`en la tarjeta `Marcas`, el mapa rellenado con datos se vuelve azul. A los valores negativos se les asigna el naranja.

Puedes que en ocasiones no quieras que el mapa sea azul. Quizá prefieras el verde o pienses que ciertos datos no se deben representar en azul, como en el caso de incendios forestales o atascos, pues resultaría muy confuso.

No hay problema, puedes cambiar la paleta de colores como antes.

5. Haz clic en `Color` en la tarjeta `Marcas` y selecciona `Editar colores`.

Para este ejemplo, debes saber qué estados tienen buenos resultados en ventas y cuáles no.

6. En la lista desplegable `Paleta`, selecciona `Rojo-verde divergente` y haz clic en `Aceptar`. Esto permite ver rápidamente los estados que tienen peores resultados y los que tienen mejores resultados.

La vista se actualiza y tienes más o menos este aspecto:

<img src="../images/03/16.png" width="400" >

Espera un momento: ¡Todo se ha vuelto rojo! ¿Qué ha ocurrido?
...
En cualquier caso, no quieres mostrar esta vista tal cual a la jefa porque no estás convencido de que los datos estén transmitiendo información útil.

7. Haz clic en el icono `Deshacer` <img src="../images/03/17.png" > de la barra de herramientas para volver a la vista azul anterior.

Sigue habiendo un problema con el color. Así todo parece perfecto, ¡ese es el problema!

A primera vista, parece que Florida es el estado con mejores resultados. Al situar el cursor sobre el marca, descubres que tienes un total de ventas de 89 474 dólares estadounidenses, comparado por ejemplo con South Carolina, cuyas ventas solo son 8482 dólares estadounidenses. Sin embargo, ¿ha resultado rentable alguno de la región sur?

8. Arrastra `Ganancias` a `Color` en la tarjeta `Marcas` para ver si encuentras una respuesta a esta pregunta.

<img src="../images/03/18.png" width="400" >

¡Así está  mejor! Puesto que a menudo las ganancias están formadas por valores positivos y negativos, Tableau selecciona automáticamente la paleta de colores divergentes `Naranja-Azu`l para mostrar rápidamente los estados con ganancias negativas y con ganancias positivas.
...
#### Revisa tu avance: observa "Crear una vista de mapa" en acción
...
Como viste en el último paso, los mapas son estupendos para visualizar los datos de manera general. El gráfico de barras ayuda a ir al grano. Para hacerlo, crea otra hoja de trabajo.

1. Haz doble clic en `Hoja 3` y asígnale el nombre `Profit Map`.

2. Haz doble clic en `Profit Map` al final del área de trabajo y selecciona `Duplicar` . Asigna el nombre `Negative Profit Bar Chart` a la nueva hoja.

3. En la hoja de trabajo `Negative Profit Bar Chart`, haz clic en `Show Me` y selecciona las barras horizontales.

`Show Me` destaca tipos de gráficos distintos según los datos que ha agregado a la vista.

> **Nota**
> 
> Cuando lo desees, puedes volver a hacer clic en `Show Me` para contraerlo.

<img src="../images/03/20.png" width="350" >

Ahora, tienes un gráfico de barras de nuevo, así de fácil.

<img src="../images/03/21.png" width="700" >

4. Para seleccionar varias barras en la izquierda, selecciona y arrastra el cursor por varias de las barras situadas entre Tennessee, North Carolina y Florida para seleccionar las barras de la izquierda. En la descripción emergente que aparece, selecciona `Mantener solamente` para centrarse en estos tres estados.

> **Nota**
> También puedes hacer clic con el botón derecho en las barras destacadas y seleccionar `Mantener solamente`.

Ten en cuenta que el campo `Inclusiones` para `State` se agrega al estante `Filtros` para indicar que ciertos estados están filtrados en la vista. El icono con dos círculos en el campo indica que este campo es un conjunto. Puedes editarlo haciendo clic con el botón derecho en el campo en el estante `Filtros` y seleccionando `Editar filtro`.

Ahora ve los datos de las ciudades en estos estados.

5. En el estante `Filas`, haz clic en el icono de suma en el campo `State` para desglosar al nivel de detalle `City`.

Aquí hay demasiada información, así que filtra la vista a las ciudades con las ganancias más negativas usando un filtro `Top _N_`.


#### Revisa tu avance: observa los pasos del 1 al 5 en acción

<img src="../images/03/22.gif" width="800" >

#### Crear un filtro _N_ superior

Puedes utilizar un filtro _N_ superior en Tableau Desktop para establecer un límite para el número de marcas que se muestran en la vista. En este caso, deseas utilizar el filtro _N_ superior para centrarse en los que tienen menos ganancias.

1. En el panel `Datos`, arrastra `City` al estante `Filtros`.
...
- Escribe `5` en el cuadro de texto para mostrar los cinco con peores resultados en el conjunto de datos.

Tableau Desktop ya ha seleccionado un campo (`Profit`) y agregación (`Suma`) para el filtro _N_ superior según los campos de la vista. Esta configuración garantiza que la vista mostrará solo las cinco ciudades con peor rendimiento por suma de ganancias.
...
- Filtros de contexto.
- Filtros de _N_ primeros.
- Filtros de dimensión.
- Filtros de medidas.

El orden en el que creas los filtros o en el que los organizas en el estante `Filtros` no modifica el orden en el que Tableau los aplica en el vista.

Lo bueno es que puedes indicarle a Tableau que cambie este orden cuando detecta que algo extraño está sucediendo a los filtros de la vista. En este ejemplo, el filtro Top _N_ se aplica a las cinco ciudades con peor rendimiento por suma de ganancias para todo el mapa, pero ninguna de esas ciudades está en el sur, por lo que el gráfico está en blanco.

Para arreglar el gráfico, agrega un filtro al contexto. Esto indicará a Tableau que filtre primero ese campo, con independencia de si se ajusta al orden de operaciones.

Pero, ¿qué campo agrega al contexto? Hay tres campos en el estante `Filtros`: `Región` (filtro de dimensión), `Ciudad` (filtro de _N_ primeros) e `Inclusiones` (País, Estado), (País, Estado) (conjunto).

Si vuelves a observar el orden de operaciones, el conjunto y el filtro de _N_ primeros se están aplicando antes que el filtro de dimensión. ¿Pero el filtro de los _N_ primeros o el filtro de conjunto se ha aplicado primero?
...
Vas por buen camino. Pero hay seis ciudades, incluida Jacksonville y Carolina del Norte, que tienen una ganancia positiva. ¿Por qué aparece en la vista una `Ciudad` con ganancias positivas si ha creado un filtro que se supone que debía eliminarla de la vista?
...
8. Arrastra `Postal Code` fuera del estante `Filas`.

La vista se actualiza y tienes más o menos este aspecto:

<img src="../images/03/24.png" width="700" >

#### Revisa tu avance: observa los pasos del 1 al 8 en acción

<img src="../images/03/25.gif" width="800" >

#### Identificar los menos rentables

Decides desglosar la vista por `Sub-Category` para identificar los productos que están mermando las ganancias. Como sabes que el campo `Sub-Category` contiene información sobre los productos vendidos por ubicación, comienza por ahí.

1. Arrastra `Sub-Category` al estante `Filas` y colócalo a la derecha de `City`.

2. Arrastra `Profit` a `Color`en la tarjeta `Marcas` para que resulte más fácil ver qué productos tienen ganancias negativas.

3. En el panel `Datos`, haz clic con el botón derecho en `Order Date` y selecciona `Mostrar filtro`.

Ahora, si lo deseas, puedes explorar los beneficios negativos para cada año y detectar rápidamente los productos que están perdiendo dinero.
...
3. En el panel `Datos`, arrastra `Profit` y `Profit Ratio` a `Etiqueta` en la tarjeta `Marcas`. Para dar formato a la `Relación de beneficios` como porcentaje, haz clic con el botón derecho en `Relación de beneficios` y selecciona `Formato`. Luego, para `Números predeterminados`, elige `Porcentaje` y establece el número de lugares decimales que deseas que se muestren en el mapa. Para este mapa, elige cero lugares decimales.
...
5. Borra `Binders`, `Machines` y `Tables` de la lista en la tarjeta de filtro `Sub-Category` en la vista.

No olvides que al agregar filtros a la vista, podrás incluir y excluir valores para destacar ciertos elementos de los datos.
...
`Binders`, `Machines` y `Tables` son definitivamente los responsables de las pérdidas en Tennessee, North Carolina y Florida, pero no en el resto de la región South. ¿Has notado cómo las ganancias disminuyen en realidad para otros estados cuando borra artículos de la tarjeta de filtro? Si, por ejemplo, conmuta Binders (Carpetas) en la tarjeta de filtro `Sub-Category`, las ganancias caen un cuatro por ciento en Arkansas. Puedes deducir que las carpetas son rentables en Arkansas.

Quieres compartir este hallazgo con el equipo y para ello quiereS que sigan los mismos pasos hechos por ti.

6. Selecciona (`Todos`) en la tarjeta de filtro `Sub-Category` para incluir todos los productos de nuevo.

Has averiguado que `Binders`, `Machines` y `Tables` son los responsables de las ganancias negativas en Tennessee, North Carolina y Florida. ¿Ahora qué?

Puedes seguir explorando los datos respondiendo a las preguntas de abajo. 

> **Nota**
>
> Si usas la vista para explorar estas preguntas, deshaz cualquier cambio antes de continuar con el tutorial `Introducción`.
...
- ¿Qué otros factores pueden contribuir a estos resultados? ¿Los minoristas de la región South están vendiendo productos con descuento?
...
- ¿Qué ocurre con `Bookcases`? Recuerda que en el paso 2 observaste que `Bookcases` no era nada rentable en otras regiones. Si profundizas en esas regiones, podrás descubrir otras soluciones.

#### Revisa tu avance: observa "comprobar los hallazgos" en acción

<img src="../images/03/28.gif" width="900" >

Ya sabes que `Machines`, `Tables` y `Binders` son productos problemáticos para el empresa. Al centrarse en el sur, comprueba que estos productos tienen un impacto variable en las ganancias. Puedes que merezca la pena comentar esto con el jefa.

A continuación, recopilarás el trabajo que has realizado hasta ahora en un dashboard para poder presentar sus hallazgos de manera clara.
